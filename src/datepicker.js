...
function isBoolean(value) {
  return (
    value === true ||
    value === false ||
    (isObjectLike$c(value) && baseGetTag$9(value) == boolTag$4)
  );
}
...
function isNumber(value) {
  return (
    typeof value == "number" ||
    (isObjectLike$b(value) && baseGetTag$8(value) == numberTag$4)
  );
}
...
function isString(value) {
  return (
    typeof value == "string" ||
    (!isArray$d(value) &&
      isObjectLike$a(value) &&
      baseGetTag$7(value) == stringTag$4)
  );
}
...
function hashHas$1(key) {
  ...
  return nativeCreate$1
    ? data2[key] !== void 0
    : hasOwnProperty$b.call(data2, key);
}
...
function assignValue$3(object, key, value) {
  ...
  if (
    !(hasOwnProperty$a.call(object, key) && eq$4(objValue, value)) ||
...
function equalArrays$2(array, other, bitmask, customizer, equalFunc, stack) {
  ...
  while (++index2 < arrLength) {
    ...
    if (seen) {
      if (
        !arraySome$1(other, function (othValue2, othIndex) {
          if (
...
if (
  ...
  getTag$5 = function (value) {
    var result = baseGetTag$1(value),
      Ctor = result == objectTag$3 ? value.constructor : void 0,
...
function isIterateeCall$3(value, index2, object) {
  ...
  if (
    type == "number"
      ? isArrayLike$3(object) && isIndex(index2, object.length)
...
var defaults = baseRest$2(function (object, sources) {
  ...
  while (++index2 < length) {
    ...
    while (++propsIndex < propsLength) {
      ...
      if (
        value === void 0 ||
        (eq$1(value, objectProto$3[key]) && !hasOwnProperty$3.call(object, key))
      ) {
...
var mergeWith$1 = createAssigner(function (
  object,
  source,
  srcIndex,
  customizer
...
const pageIsBetweenPages = (page, fromPage, toPage) =>
  (page || false) &&
...
const mixinOptionalProps = (source, target, props) => {
  ...
  props.forEach((p) => {
    ...
    if (Object.prototype.hasOwnProperty.call(source, name)) {
      ...
      target[name] =
        mixin && isObject(value)
          ? __spreadValues(__spreadValues({}, mixin), value)
          : value;
      assigned.push(name);
    }
  });
...
const _sfc_main$9 = {
  ...
  computed: {
    content() {
      return (
        (isFunction_1(this.$slots.default) &&
          this.$slots.default({
            direction: this.direction,
  ...
  watch: {
    opts(val, oldVal) {
      if (oldVal && oldVal.callback) {
        oldVal.callback(
          __spreadProps(__spreadValues({}, oldVal), {
            completed: !val,
...
class Theme {
  ...
  normalizeHighlight(config) {
    ...
    toPairs_1(highlight).forEach(([_, targetConfig]) => {
      ...
      targetConfig.style = __spreadValues(
        __spreadValues({}, this.getHighlightBgStyle(c)),
        targetConfig.style
      );
      targetConfig.contentStyle = __spreadValues(
        __spreadValues({}, this.getHighlightContentStyle(c)),
  ...
  normalizeNonHighlight(type, config, styleFn) {
    ...
    toPairs_1(attr).forEach(([_, targetConfig]) => {
      ...
      targetConfig.style = __spreadValues(
        __spreadValues({}, styleFn(targetConfig)),
        targetConfig.style
      );
    });
    return attr;
...
function getTimezoneOffsetInMilliseconds(dirtyDate) {
  ...
  var millisecondsPartOfTimezoneOffset = hasNegativeUTCOffset
    ? (MILLISECONDS_IN_MINUTE$2 + getDateMillisecondsPart(date)) %
      MILLISECONDS_IN_MINUTE$2
    : getDateMillisecondsPart(date);
  return (
    baseTimezoneOffset * MILLISECONDS_IN_MINUTE$2 +
...
class DateInfo {
  constructor(config, { order = 0, locale, isFullDay } = {}) {
    ...
    let end = null;
    if (config.start) {
      start = this.locale.normalizeDate(
        config.start,
    ...
    if (config.on) {
      const or = (isArrayLikeObject_1(config.on) ? config.on : [config.on])
        .map((o) => {
  ...
  static get patterns() {
    return {
      ...
      days: {
        validate: (days) =>
          isArrayLikeObject_1(days) ? days : [parseInt(days, 10)],
        test: (day, days) =>
          days.includes(day.day) || days.includes(-day.dayFromEnd),
      },
      weekdays: {
        validate: (weekdays2) =>
          isArrayLikeObject_1(weekdays2)
            ? weekdays2
      ...
      ordinalWeekdays: {
        validate: (ordinalWeekdays) =>
          Object.keys(ordinalWeekdays).reduce((obj, ck) => {
            ...
            obj[ck] = isArrayLikeObject_1(weekdays2)
              ? weekdays2
              : [parseInt(weekdays2, 10)];
            return obj;
          }, {}),
      ...
      weeks: {
        validate: (weeks) =>
          isArrayLikeObject_1(weeks) ? weeks : [parseInt(weeks, 10)],
        test: (day, weeks) =>
          weeks.includes(day.week) || weeks.includes(-day.weekFromEnd),
      },
      months: {
        validate: (months) =>
          isArrayLikeObject_1(months) ? months : [parseInt(months, 10)],
        test: (day, months) => months.includes(day.month),
      },
      years: {
        validate: (years) =>
          isArrayLikeObject_1(years) ? years : [parseInt(years, 10)],
...
function resolveConfig(config, locales2) {
  ...
  const defLocale = __spreadProps(
    __spreadValues(__spreadValues({}, locales2["en-IE"]), locales2[id]),
    { id }
  );
...
class Locale {
  constructor(config, { locales: locales$1 = locales, timezone } = {}) {
    const {
      ...
      firstDayOfWeek,
      masks: masks2,
    } = resolveConfig(config, locales$1);
    this.id = id;
  ...
  normalizeDate(d, config = {}) {
    ...
    if (result && patch) {
      ...
      const parts = __spreadValues(
        __spreadValues({}, this.getDateParts(fillDate)),
  ...
  normalizeDates(dates, opts) {
    opts = opts || {};
    opts.locale = this;
    return (isArrayLikeObject_1(dates) ? dates : [dates])
    ...
    createElementBlock("div", _hoisted_1$4, [
      ...
      createElementVNode("div", _hoisted_3$3, [
        renderSlot(_ctx.$slots, "default", {}, () => [
          createTextVNode(
            toDisplayString(
              $props.attribute.popover
                ? $props.attribute.popover.label
                : "No content provided"
            ),
            1
          ),
      ...
      [
          ...
          createElementVNode(
            "span",
            {
              ...
              onClick:
                _cache[0] ||
                (_cache[0] = (...args) =>
                  $options.movePrev && $options.movePrev(...args)),
              onKeydown:
                _cache[1] ||
                (_cache[1] = (e) => $data.onSpaceOrEnter(e, $options.movePrev)),
            },
            [
          ...
          createElementVNode(
            "span",
            {
              ...
              onClick:
                _cache[2] ||
                (_cache[2] = (...args) =>
                  $options.toggleMode && $options.toggleMode(...args)),
              onKeydown:
                _cache[3] ||
                (_cache[3] = (e) =>
                  $data.onSpaceOrEnter(e, $options.toggleMode)),
            },
            toDisplayString($options.title),
            33
          ),
          createElementVNode(
            "span",
            {
              ...
              onClick:
                _cache[4] ||
                (_cache[4] = (...args) =>
                  $options.moveNext && $options.moveNext(...args)),
              onKeydown:
                _cache[5] ||
                (_cache[5] = (e) => $data.onSpaceOrEnter(e, $options.moveNext)),
            },
            [
        ...
        createElementVNode("div", _hoisted_5$1, [
          ...
          createElementBlock(
            Fragment,
            null,
            renderList($options.activeItems, (item) => {
              return (
                openBlock(),
                createElementBlock(
                  "span",
...
const _sfc_main$5 = {
  ...
  render() {
    ...
    const contentLayer = () =>
      ...
      h(
        ...
        __spreadProps(
          __spreadValues(
            __spreadProps(__spreadValues({}, this.dayContentProps), {
  ...
  methods: {
    ...
    processHighlight(
      { key, highlight },
      { isDate: isDate2, isComplex, onStart, onEnd, onStartAndEnd },
      { backgrounds, content }
    ) {
      if (!highlight) return;
  ...
  render() {
    ...
    this.page.days.forEach((day, i) => {
      ...
      if (
        ...
        (showWeeknumbersRight && mod === daysInWeek2)
      ) {
...
const addHorizontalSwipeHandler = (
  element,
  handler,
  {
  ...
  function touchEnd(e) {
    ...
    if (deltaTime < maxSwipeTime2) {
      if (
        Math.abs(deltaX) >= minHorizontalSwipeDistance2 &&
        Math.abs(deltaY) <= maxVerticalSwipeDistance2
      ) {
...
const _sfc_main$3 = {
  ...
  methods: {
    ...
    showPageRange(range, opts) {
      ...
      return this.refreshPages(
        __spreadProps(__spreadValues({}, opts), { page })
      );
    },
  ...
  methods: {
    updateValue(hours, minutes = this.minutes) {
      if (hours !== this.hours || minutes !== this.minutes) {
        this.$emit(
          "update:modelValue",
          __spreadProps(__spreadValues({}, this.modelValue), {
            hours,
            minutes,
            seconds: 0,
            milliseconds: 0,
      ...
      [
        ...
        createElementVNode("div", _hoisted_2, [
            ...
            ? (openBlock(),
              createElementBlock("div", _hoisted_3, [
                createElementVNode(
                  "span",
                  _hoisted_4,
                  toDisplayString($props.locale.format($options.date, "WWW")),
                  1
                ),
                createElementVNode(
                  "span",
                  _hoisted_5,
                  toDisplayString($props.locale.format($options.date, "MMM")),
                  1
                ),
                createElementVNode(
                  "span",
                  _hoisted_6,
                  toDisplayString($props.locale.format($options.date, "D")),
                  1
                ),
                createElementVNode(
                  "span",
                  _hoisted_7,
                  toDisplayString($props.locale.format($options.date, "YYYY")),
                  1
                ),
              ]))
            : createCommentVNode("", true),
          createElementVNode("div", _hoisted_8, [
            createVNode(
              _component_time_select,
              {
                modelValue: $options.hours,
                "onUpdate:modelValue":
                  _cache[0] ||
                  (_cache[0] = ($event) => ($options.hours = $event)),
            ...
            createVNode(
              _component_time_select,
              {
                ...
                "onUpdate:modelValue":
                  _cache[1] ||
                  (_cache[1] = ($event) => ($options.minutes = $event)),
                modelModifiers: { number: true },
...
const _sfc_main = {
      ...
      const events = [true, false].map((isStart) =>
        __spreadValues(
          ...
          getPopoverTriggerEvents(
            __spreadProps(__spreadValues({}, this.popover_), {
              id: this.datePickerPopoverId,
              callback: (e) => {
  ...
  methods: {
    ...
    normalizeConfig(config, baseConfig = this.modelConfig_) {
      config = isArrayLikeObject_1(config)
        ? config
    ...
    normalizeValue(value, config, patch, rangePriority) {
      ...
      if (this.isRange) {
        ...
        result.start = this.normalizeDate(
          start,
          __spreadProps(__spreadValues({}, config[0]), {
            fillDate: (this.value_ && this.value_.start) || config[0].fillDate,
            patch,
          })
        );
        const end = value.start > value.end ? value.start : value.end;
        result.end = this.normalizeDate(
          end,
          __spreadProps(__spreadValues({}, config[1]), {
...
var screensPlugin = {
  install: (app, screens) => {
    ...
    const refreshMatches = () => {
      state2.matches = toPairs_1(state2.queries)
